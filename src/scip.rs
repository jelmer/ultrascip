...
fn index_python(
    ...
    // scip-python falls back to the directory name and version 0.0 when it
    // cannot determine the project name and version itself (e.g. a dynamic
    // version), which would bake a wrong package name into every emitted
    // symbol. Resolve them up front via the project's PEP 517 wheel metadata
